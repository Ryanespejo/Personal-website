...
RAPIDAPI_KEY = os.getenv("RAPIDAPI_KEY") or os.getenv("RAPIDAPI_TENNIS_KEY", "")
...
RAPID_STATUS_KEY = "rapidapi_status"
...
def _set_rapid_status(ok: bool, message: str = ""):
    _data_cache[RAPID_STATUS_KEY] = {
        "ok": ok,
...
def _rapid_fetch_json(path: str, params: dict | None = None) -> dict:
    if not RAPIDAPI_KEY:
        _set_rapid_status(False, "RAPIDAPI_KEY is not set")
        return {}
    q = ""
    if params:
    ...
    try:
        ...
        try:
            ...
            pass
        msg = f"HTTP {e.code} from RapidAPI"
        low = (body or "").lower()
        if "api key" in low or "unauthorized" in low or e.code in (401, 403):
            msg = "RapidAPI authorization failed. Check RAPIDAPI_KEY and RAPIDAPI_TENNIS_HOST"
        _set_rapid_status(False, msg)
        return {}
    except Exception:
        _set_rapid_status(False, "RapidAPI request failed")
        return {}
