...
def normalize(comp: dict, event: dict):
    ...
    for c in competitors:
        ...
        # Inning scores from linescores
        linescores = c.get('linescores') or []
        innings = []
        for ls in linescores:
            val = ls.get('value')
            innings.append(str(int(float(val))) if val is not None else '—')

        # Hits and errors from statistics
        hits = 0
        errors = 0
        ...
        for stat in stats:
            ...
            if stat_name in ('hits', 'H'):
                try:
                    hits = int(stat_val)
                except (ValueError, TypeError):
                    hits = 0
            elif stat_name in ('errors', 'E'):
                try:
                    errors = int(stat_val)
                except (ValueError, TypeError):
                    errors = 0
...
class handler(BaseHTTPRequestHandler):
    ...
    def _send_json(self, code: int, data: dict):
        ...
        self.send_header('Access-Control-Allow-Origin', '*')
        self.send_header('Cache-Control', 'no-cache, no-store, must-revalidable')
        self.end_headers()
